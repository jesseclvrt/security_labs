...
oops:   fprintf(stderr, "\nUsage: %s plainImage stegoImage stegoData\n\n", argv[0]);
    ...
    if((imageBytes - START_FROM - 27 - 64) <= 0) //Image must be at least 100+27+64 bytes
    {
    ...
    if((dataBytes << 3) >= (imageBytes - START_FROM - 27 - 64)) //databytes<<3 cannot be larger than image-100-27-64
    {
    ...
    for(i = 0; i < 8; ++i)
    {
        for(j = 0; j < 8; ++j) //encodes 1 byte
        {
            x = fscanf(in, "%c", &temp); //get a file btye
            if(x != 1)
            {
                fprintf(stderr, "\nError in file %s\n\n", infname);
                exit(0);
            }
            //select most significant bits, encode lsb with data bit
            temp = (temp & 0xfe) ^ ((ttt >> j) & 0x1);
            //outputs encoded byte
            fprintf(out, "%c", temp);
            ++imageBytesWritten;
        }
    }
